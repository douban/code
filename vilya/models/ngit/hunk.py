...
from ellen.utils.mdiff import hunk2, mdiff2

from vilya.models.consts import LINECOMMENT_INDEX_EMPTY
from vilya.models.ngit.line import Line


class Hunk(object):

    # FIXME: old_lines/new_lines 不包含 'No newline at end of file'，
    # 导致行号对不上，也导致review评论显示不出来
    def __init__(self, patch, hunk=None, start_pos=None, **kw):
        ...
        self._mdiff_lines = None

    @property
    def mdiff_lines(self):
        if self._mdiff_lines is None:
            self._mdiff_lines = mdiff2(self._hunk['lines'])
        return self._mdiff_lines
    ...
    def mdiff(self, side_by_side=None):
        ...
        if not self._hunk:
            return []
        return hunk2(self._hunk['lines'], self.mdiff_lines,
    ...
    def _side_lines(self, side=None):
        def filter_linecomments(_line):
            ...
            comments_by_pos = self.patch.comments_by_pos.get(
                _line.pos_in_patch)
            if comments_by_line:
