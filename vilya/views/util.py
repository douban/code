...
RE_MOBILE = re.compile(r"android.+mobile|samsumg|avantgo|bada\\/|blackberry|"
                       ...
                       re.I | re.M)
RE_OS_IN_USER_AGENT = re.compile("(windows|mac(?= os x)|"
                                 "iphone|android|linux)(?= )")
RE_BROWSER_IN_USER_AGENT = re.compile("((?<= )msie \d{1,2}|"
                                      "(?<=)firefox\/\d{1,2}|"
                                      "webkit(?=[ \/]))|"
                                      "\\bopera\\b")
