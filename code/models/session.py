...
from code.libs.session import (randbytes2,
                               get_site_cookie,
                               format_cookie_date)
from code.config import (SESSION_EXPIRE_DAYS,
                         SESSION_DOMAIN,
                         SESSION_COOKIE_NAME)
...
class SessionMixin(object):
    ...
    def clear_session(self):
        ...
        self.save()
        #store.execute("update users set session_id=NULL where id=%s",
        #              self.id)
        #store.commit()
        self.session_id = None
    ...
    def create_session(self):
        ...
        self.save()
        #store.execute("update users set session_id=_latin1%s, "
        #              "session_expired_at=%s "
        #              "where id=%s",
        #              (session_id, expired_time, self.id))
        #store.commit()
        #self.session_expired_at = expired_time
        #self.session_id = session_id
