...
参见：
http://svn.douban.com/projects/shire/wiki/RebuildFrontCode_Tribe
'''
...
import re
import functools
from code.config import CODE_DIR
...
#from config.conf import STATIC_HOST
def css_js_url(path):
    return path
...
class StaticManager(object):
    ...
    def __init__(self, static_site=''):
        ...
        self.static_dir = join(CODE_DIR, STATIC_FILE_DIR, self.static_type)
...
class ModuleStatic(object):
    ...
    def __init__(self, static_path, converters, final_converter=None,
        ...
        filename = self.path.rsplit('.', 1)[0].lstrip('/')
...
def import_converter(module, file_type):
    ...
    def get_content(match):
        ...
        if not path[0] in './':
            raise Exception("import path should start with . or / %s imported in %s" % (path,
                                                                                        module.path))
